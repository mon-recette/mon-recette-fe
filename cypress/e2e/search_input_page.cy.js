// beforeEach(() => {
//   cy.fixture('chicken_search.json').as('chickenData').then((data) => {
...
describe('Should display the home page with search', () => {
  ...
  it('should be able to search for an ingredient and display recipes', () => {
    cy.intercept('GET', 'http://localhost:3000/api/v1/recipes?search=chicken', (req) => {
      req.reply({
        fixture: 'chicken_search.json',
      });
    }).as('searchRequest');
    ...
    //   cy.log('Search request intercepted:', interception);
    // });
